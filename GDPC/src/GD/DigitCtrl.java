...
public class DigitCtrl extends Controller
{
	...
	public DigitCtrl(CrossBitmap images[])
	{
		imgs = images;
	}
	...
	public void onSetCurrent()
	{
		...
		switch (state)
		{
			...
			/*	statusLabels = new GUILabel[5];
				...
				statusLabels[2] = new GUILabel(0, 35, 0, 10, "OCR wartości osi", 120, 25);
		...
		if (state == STATE_DIGITALIZE)
		{
			...
			if (statusLabels != null)
				for (GUILabel lab: statusLabels)
				{
					...
					g.gStretchAlignBitmap(0, Dim.H(1) + lab.getY(), lab.getX()-10, Dim.H(8), 1, Dim.H(8), bit, Comm.STRETCH_V_PROP, Comm.ALIGN_RIGHT);
					i++;
				}
		}*/
	...
	{/*
        ...
        for (int y = 0; y< outBit.getHeight(); y++)
          for (int x = 0; x< outBit.getWidth(); x++)
            outBit.setRGB(x, y, Colors.WHITE);        
