 ...
 */
public class MenuCtrll extends Controller
{
	...
	public void onGUIAction(GUIAction e)
	{
		switch (e.action)
		{
			case GUIAction.BUTTON_CLICK:
				...
				if (e.id.equals(ID_BUT_DIGIT))
				{
					...
					Main.main.mainCrtl.chageController(new DigitCtrll(null));
			break;
		}
	}
        ...
        if (JFileChooser.APPROVE_OPTION == fc.showOpenDialog(null)) {
            try {
                Main.main.mainCrtl.chageController(new CutCtrll(fc.getSelectedFile()));
            } catch (IOException ex) {
                System.err.println("Wybieraj jakis lepszy ten obrazek");
                Logger.getLogger(MenuCtrll.class.getName()).log(Level.SEVERE, null, ex);
            }
        } else {
