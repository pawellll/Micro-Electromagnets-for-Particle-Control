...
public class GUIImage extends GUIComponent
{
    ...
    double nh = img.getHeight();
		...
		{
			nh = (int)((double)nh*(double)w/nw);
			nw = w;
		}
		else
		if (stretchFlag == Comm.STRETCH_V_PROP)
		{
			nw = (int)((double)nw*(double)h/nh);
			nh = h;
		}
		else
		{			
	...
	public void onPaint(DrawingInterface g, int width, int height)
	{
		...
		if (selectable && selectStarted)
		{
			...
			g.gFillRectangle((int)Math.min(startX, endX)+2, (int)Math.min(startY, endY)+2, (int)Math.abs(endX-startX)-4, (int)(Math.abs(endY-startY))-4);
